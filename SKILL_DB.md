...
- **`agent_tools.delete_order` soft-delete 후 calendar sync 누락 (2026-05-04 수정)**: 주문을 영구 삭제하는 LLM 도구 `agent_tools.delete_order` 는 `order_service.py` 에 동등 service 메서드가 없고 agent_tools 에만 동기 함수로 존재한다. 기존 흐름은 ① status 가 CANCELLED/COMPLETED 가 아니면 → status='CANCELLED' UPDATE → `_sync_calendar_events_for_order_id` 호출, ② `deleted_at = NOW()` UPDATE 였는데, ②에는 calendar sync 호출이 없었다. 결과: 사용자가 이미 CANCELLED 또는 COMPLETED 인 주문을 삭제하면 ① 분기를 거치지 않고 ② 만 실행되어 active calendar_events 가 orphan 으로 남는다. (TERMINAL_ORDER_STATUSES 패치가 sync 함수 본체에 있었지만, sync 호출 자체가 없으면 무용지물.) 해결: ② 직후에도 `_sync_calendar_events_for_order_id(order_id)` 호출 추가. `order_service.sync_calendar_events_for_order_id` 는 active order 조회 → 없으면 `_soft_delete_calendar_events_for_order_sync(order_id)` fallback 이라 `deleted_at` 직후 호출에도 정상 동작 (active 주문 미발견 → calendar_events 직접 soft-delete 경로). sync 실패가 delete 자체를 막지 않도록 try/except 로 감싸 로그만 남김 (`cancel_order` 의 chat-event 패턴 동일).
  - **검증 패턴**: 주문 mutation 함수 추가/수정 시 "DB UPDATE 직후 `_sync_calendar_events_for_order_id` 호출이 모든 분기에 존재하는가" 를 AST 로 카운트 검증. 한 함수 내 `if/else` 분기마다 sync 호출이 빠져있는지 확인 필요.
  - **다른 mutation 점검 결과**: `agent_tools.create_order` (line 867), `update_status` (line 867), `update_order` (line 946) 는 단일 분기라 호출 누락 없음. delete_order 만 status 분기 + deleted_at 분기 두 단계라 sync 호출도 두 곳 필요했다.
