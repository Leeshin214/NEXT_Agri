...
class OrderService:
    ...
    async def cancel_order(
        ...
        # 구매자 대체 거래처 자동 탐색 (AI 대화 히스토리 + 알림) — fire-and-forget.
        # supabase 다중 조회·insert·notification 을 모두 await 으로 직렬 처리하면
        # cancel API 응답이 지연되어 클라이언트가 "Failed to fetch" 로 떨어진다.
        async def _trigger_safe():
            try:
                await self._trigger_alternative_partner_for_cancel(
                    updated_order or order, order_id
                )
            except Exception as e:
                print(
                    f"[order_service.cancel_order] 대체 거래처 탐색 실패 (무시): "
                    f"{type(e).__name__}: {e}"
                )

        asyncio.create_task(_trigger_safe())

        return updated_order

    async def _trigger_alternative_partner_for_cancel(self, order: dict, order_id) -> None:
        """주문 취소 시 구매자 대체 거래처 탐색 + ai_conversations + 알림."""
        ...
        items_res = await asyncio.to_thread(
            lambda: _sb.table("order_items").select("product_id, quantity").eq("order_id", str(order_id)).limit(1).execute()
        )
        ...
        alternatives_result = await asyncio.to_thread(
            find_alternative_partners, buyer_id_str, "BUYER", product_category, "주문 취소"
        )
