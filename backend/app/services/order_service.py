        ...
        if new_status == "CANCELLED":
            try:
                ...
                if updated_order is not None:
                    updated_order["buyer_notified"] = True
            except Exception as e:
                print(f"[order_service.update_status] 대체 거래처 탐색 실패 (무시): {type(e).__name__}: {e}")
                if updated_order is not None:
                    updated_order["buyer_notified"] = False
