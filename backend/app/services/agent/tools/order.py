...
@tool(
    ...
    description=(
        ...
        "[절대 주의 3 — 가격] unit_price 는 (1) 사용자가 가격을 명시한 경우 그 가격을 그대로, (2) 가격 미명시 시 반드시 find_sellers_by_product / check_stock 응답에서 '사용자가 지정한 그 셀러의 그 상품의 price_per_unit' 만 사용한다. "
        "다른 셀러의 가격이나 일반 시장 추정 가격을 절대 사용하지 마라 (예: 판매자A 감자=20,000원/kg, 판매자B 감자=3,000원/kg 일 때 사용자가 'A에게 주문'했는데 3,000을 unit_price 로 넣으면 자동 카운터오퍼가 잘못 발사된다). "
        "사용자가 가격을 명시했지만 단위가 모호한 경우(예: '21만원에 감자 7kg' — kg당? 총가?), 둘 다 합리적이면 도구 호출 X, 사용자에게 '말씀하신 21만원이 kg당 가격인지, 7kg 전체 가격인지 알려주세요' 라고 먼저 되묻고 답변을 받은 뒤 호출한다. "
        "[hard guard 안내] 백엔드는 사용자 발화 ↔ tool 인자 일관성 검증 가드를 활성화한다 — 사용자 발화에 날짜 표현이 없는데 delivery_date 가 채워지거나, 가격 미명시인데 unit_price 가 표시가와 다르면 즉시 거부 응답이 반환된다. 거부 응답을 받으면 결과를 '주문 실패'로 안내하지 말고 자연체로 사용자에게 부족한 정보를 되물어라."
    ),
    parameters={
        "type": "object",
        "properties": {
            ...
            "unit_price": {
                ...
                "description": (
                    "단위당 가격 (원). "
                    "[가격 미명시 시] 사용자가 가격을 말하지 않았다면 find_sellers_by_product / check_stock 응답 중 "
                    "사용자가 지정한 '그 셀러의 그 상품의 price_per_unit' 을 정확히 그대로 입력한다. "
                    "다른 셀러의 가격이나 시장 추정값 사용 금지 — 백엔드 hard guard 가 표시가와 다르면 즉시 거부한다. "
                    "[가격 명시 시 — 단위 모호] '21만원에 감자 7kg 주문' 같이 단위가 불명확한 경우, "
                    "두 해석(kg당 21만 = 147만원 / 총 21만 = kg당 3만원) 을 product.price_per_unit 과 비교해 "
                    "한쪽만 합리적이면 그것을 채택. 둘 다 모호하면 도구 호출 X, 사용자에게 단위(kg당/총가) 를 되묻고 답을 받은 뒤 호출."
                ),
            },
            "delivery_date": {
                "type": "string",
                "description": (
                    ...
                    "이 도구 자체를 호출하지 말고, '납품일은 언제로 할까요? (예: 5월 20일)' 라고 먼저 물어봐라. "
                    "백엔드 hard guard: 사용자 발화에 날짜 표현(○월 ○일/내일/다음 주/요일/YYYY-MM-DD 등) 이 없는데 "
                    "이 필드가 채워져 호출되면 즉시 거부된다. LLM 임의 추정(예: '오늘+7일') 절대 금지."
                ),
            },
            "delivery_address": {
                "type": "string",
