...
async def inventory_order_node(state: AgentState) -> dict:
    ...
    for tool_call in pending_tools:
        ...
        # LLM이 id 파라미터를 null이거나 UUID가 아닌 값으로 보낸 경우 자동 교체
        import inspect as _inspect, re as _re2
        from app.services.agent_tools import TOOL_FUNCTION_MAP as _TFMAP
        user_id = state.get("user_id", "")
        _uuid_re = _re2.compile(r'^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$', _re2.I)
        _func = _TFMAP.get(tool_name)
        if _func:
            _params = set(_inspect.signature(_func).parameters.keys())
            for id_field in ("seller_id", "user_id", "buyer_id"):
                if id_field in _params:
    ...
    async def run(
        ...
        initial_state: AgentState = {
            ...
            "messages": [
                ...
                *[m for m in history if m.get("role") in ("user", "assistant")],  # tool 결과 제외
