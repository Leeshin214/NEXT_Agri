    ...
    <div>
        ...
        <div className="lg:col-span-3 rounded-xl bg-white p-6 shadow-sm">
          ...
          <div className="grid grid-cols-7 gap-px">
            ...
            {days.map((day) => {
              ...
              return (
                <div key={day} onClick={() => setSelectedDate(dateStr)} className={cn('min-h-[80px] cursor-pointer rounded-lg border p-1.5 transition-colors', selectedDate === dateStr ? 'border-primary-500 bg-primary-50' : 'border-transparent hover:bg-gray-50')}>
                  <span className={cn('inline-flex h-6 w-6 items-center justify-center rounded-full text-xs', isToday && 'bg-primary-600 text-white font-bold')}>{day}</span>
                  <div className="mt-1 space-y-0.5">
                    {dayEvents.slice(0, 2).map((ev) => <div key={ev.id} className={cn('truncate rounded px-1 py-0.5 text-[10px] text-white', eventTypeColors[ev.event_type] || 'bg-gray-500')}>{ev.title}</div>)}
                    {dayEvents.length > 2 && <p className="text-[10px] text-gray-400">+{dayEvents.length - 2}개</p>}
                  </div>
                </div>
              );
            })}
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="일정 추가" size="sm">
        <form onSubmit={handleCreateEvent} className="space-y-4">
          <div><label className="mb-1 block text-sm font-medium text-gray-700">제목</label><input name="title" required className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500" /></div>
          <div><label className="mb-1 block text-sm font-medium text-gray-700">유형</label><select name="event_type" className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500">{EVENT_TYPE_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}</select></div>
          <div><label className="mb-1 block text-sm font-medium text-gray-700">설명</label><textarea name="description" rows={3} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500" /></div>
          <div className="flex justify-end gap-2">
            ...
            <button type="submit" disabled={createEvent.isPending} className="rounded-lg bg-primary-600 px-4 py-2 text-sm text-white hover:bg-primary-700 disabled:opacity-50">{createEvent.isPending ? '저장 중...' : '저장'}</button>
          </div>
        </form>
      </Modal>
    </div>
