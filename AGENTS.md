...
실제 sub-agent는 `.Codex/agents/` 에 정의되어 있으며, Codex가 작업 유형에 따라 자동 위임한다.

| Agent | 파일 | 담당 범위 |
|-------|------|---------|
| frontend-agent | `.Codex/agents/frontend-agent.md` | Next.js 페이지·컴포넌트, Tailwind, Zustand, 라우팅, UI 전체 |
| backend-agent | `.Codex/agents/backend-agent.md` | FastAPI, SQLAlchemy, Pydantic, DB 마이그레이션, RLS, Auth |
| ai-agent | `.Codex/agents/ai-agent.md` | Codex API 연동, 스트리밍, 프롬프트 설계, AI 기능 확장 |
| validator-agent | `.Codex/agents/validator-agent.md` | 코드 수정 후 자동 검증 — TS 컴파일, Python 문법, API 계약 불일치 탐지 |
